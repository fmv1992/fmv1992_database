...
test: validate_docker_compose test_semantic_python

test_semantic_python:
	@# `*.tests_are_done` exists to ensure that tests executed ok.
	DOCKER_CMD='bash -c '"'"'python3 -m unittest discover -vvv --top-level-directory ./test --start-directory ./test/unit_test/fmv1992_database && test -f /tmp/.$@.tests_are_done && rm /tmp/.$@.tests_are_done'"'" make docker_run
